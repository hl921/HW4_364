...
app.config["SQLALCHEMY_DATABASE_URI"] = os.environ.get('DATABASE_URL') or "postgresql://localhost/haeleeHW4db" # TODO 364: You should edit this to correspond to the database name YOURUNIQNAMEHW4db and create the database of that name (with whatever your uniqname is; for example, my database would be jczettaHW4db). You may also need to edit the database URL further if your computer requires a password for you to run this.
...
tags = db.Table('tags', db.Column('gif_id', db.Integer, db.ForeignKey('gifs.id')), db.Column('search_id', db.Integer, db.ForeignKey('searchterm.id')))
...
user_collection = db.Table('user_collection', db.Column('gif_id', db.Integer, db.ForeignKey('gifs.id')), db.Column('collections_id', db.Integer, db.ForeignKey('PersonalGifCollection.id')))
...
class User(UserMixin, db.Model):
    ...
    # gifs = db.relationship('Gif',backref='User')
...
def get_gifs_from_giphy(search_string):
    ...
    param = {"api_key":api_key, "q":str(search_string), "limit":5}
...
def get_or_create_search_term(db_session, term):
    ...
    if s:
        ...
        for gif in gif_list:
            gif = get_or_create_gif(db_session, gif['title'], gif['embed_url'])
            s.gifs.append(gif)
        db_session.add(s)
        db_session.commit()
    return s
...
def get_or_create_collection(db_session, name, current_user, gif_list=[]):
    ...
    collection = db_session.query(PersonalGifCollection).filter_by(name=name,id=current_user.id).first()
    if collection:
        return collection
    else:
...
def create_collection():
    ...
    if request.method == 'POST':
        ...
        get_or_create_collection(db.session,current_user=current_user, name=form.name.data, gif_list=gif_objects)
